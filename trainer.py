...
class HeteroTrainer:
    ...
    def validate(self, loader, desc='Val'):
        ...
        for batch in tqdm(loader, desc=f'[{desc}]', leave=False):
            ...
            # ❌ REMOVE this conversion - edge indices are already local!
            # batch['playlist','track_in_playlist','track'].pos_edge_label_index = (
            #     edge_label_index_global_to_local(...)
            # )
            
            # Just call model directly
            pos_pred = self.model(batch)
            pos_label = batch['playlist', 'track_in_playlist', 'track'].pos_edge_label
            
            neg_edge_index, neg_edge_label = sample_negatives(batch, device=self.device, neg_ratio=3)
            neg_pred = self.model(batch, neg_edge_index)
...
if __name__ == "__main__":
    ...
    args = {
        ...
        'epochs': 50,              # ← INCREASE from 15 (now we can afford it)
        'neg_ratio': 3,            # ← Slightly increase (was 1, now 2)
        'loss_fn': 'BPR',
        'dropout': 0.3,
        'neg_sampling': 'random',
        'hard_neg_every': 10,
    ...
    trainer.scheduler = scheduler  # Attach scheduler
    # Train
    print(f"\n{'='*70}")
